...
void replacePlayer(Squad* squad, PlayerInventory* inv, const char* existing_name, const char* new_name) {
    int slot_idx = -1;
    for (int i = 0; i < SQUAD_SIZE; i++) {
        if (squad->slots[i].is_filled && isPartialMatch(squad->slots[i].player_name, existing_name)) {
            slot_idx = i;
            break;
        }
    }
    ...
    strncpy(squad->slots[slot_idx].player_name, new_player->name, MAX_NAME_LEN - 1);
    ...
    printf("Replaced '%s' with '%s' (Rating: %d) in slot %d [%s].\n",
           ...
           slot_idx + 1, squad->slots[slot_idx].position_type);
}
